...
possible_path1_levels = [ ''.join(np.choice( string.ascii_lowercase , 3)) ]
...
def gen_nletter_word(nletters):
    ...
    return ''.join(np.choice( string.ascii_lowercase , nletters))
...
num_path0_levels = np.choice(range(MIN_NUM_PATH0_LEVELS, MAX_NUM_PATH0_LEVELS), 1)
domain_structure_gendict['path0_levels'] = np.choice(possible_path0_levels, num_path0_levels)

for p0l in domain_structure_gendict['path0_levels']:
    num_path1_levels_this = np.choice(range(MIN_NUM_DOC_LEVELS, MAX_NUM_DOC_LEVELS), 1)
    for p1l in num_path1_levels_this:
        ...
        urls.append( domain_structure_gendict['protocol'] + '://' + domain_structure_gendict['subdomain'] + \
                           ...
                           '/' + p1l + '/' + doc_name + '.' + domain_structure_gendict['document_ext'] )
...
class User(object):
    def __init__(self, init_time, CLICK_DECAY_MEAN=10, TIME_DECAY_MEAN_SEC=30):
        ...
        self.n_clicks_term = max(1, np.round(np.choice(np.random.exponential(self.CLICK_DECAY_MEAN))))
        self.is_active = True
    ...
    def _init_props(self):
        ...
        self.next_click_time = self.last_click_ts + dt.timedelta(seconds=np.random.exponential(self.TIME_DECAY_MEAN_SEC))
    ...
    def step(self):
        ...
        if self.n_clicks >= self.n_clicks_term:
            ...
            return
        self.current_url = np.random.choice(urls, 1)
        self.last_click_time = self.next_click
        self.next_click_time = self.last_click_ts + dt.timedelta(seconds=np.random.exponential(self.TIME_DECAY_MEAN_SEC))



for u in np.choice(range(MIN_NUM_CONCUR_USERS, MAX_NUM_CONCUR_USERS)):
...
for click in range(N_CLICKS_TOTAL):
    active_users = list(sorted(active_users, key= lamda u: u.next_click_time) )

    u = active_users.pop()
    u.step()

    clicks.append( { 'uid': u.uid, 'ts': u.last_click, 'url': u.current_url }  )
    if u.is_active:
